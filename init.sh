...
[init] 骨架铺设完成。接下来：
  ...
  2. 按自己的项目改 team.yaml（角色、agent、handoff、config_files、anchor 风格）。
     字段注释见 core/team.schema.yaml；样例见 examples/README.md。
  3. 用你的 agent 打开项目，运行 /setup-team 生成各工具适配文件（CLAUDE.md / AGENTS.md）。
  4. 确认编制无误后，开始第一个增量（找前台 /setup-team 分拣）。
