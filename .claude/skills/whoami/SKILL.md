...
## 干活
1. 读项目根 `team.yaml`：本项目有哪几个 role、本 agent（看 CLAUDE.md 里声明的 agent 名）担任哪些 role、交接逻辑。
...
- 产品/增量名以用户给你的转达 prompt 为准；不知道就如实说"本轮产品名未告知"。
- 结尾按 CLAUDE.md 里配置的锚点风格署名——**这本身就是自检的一部分**：署得出且 role 对，说明身份在线。
