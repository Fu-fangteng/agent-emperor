...
```
.claude/skills/
├─ setup-team/SKILL.md   配置向导（问 5 件事 → 写 team.yaml → 调 generate.py）
├─ whoami/SKILL.md       当场自检：我是谁、轮到我没、当前 STATUS
├─ plan/SKILL.md         planner 触发器
├─ review/SKILL.md       reviewer 触发器
└─ sync/SKILL.md         只读对齐状态
```

clone 本仓即可用 `/setup-team`、`/whoami`、`/plan`、`/review`、`/sync`。
